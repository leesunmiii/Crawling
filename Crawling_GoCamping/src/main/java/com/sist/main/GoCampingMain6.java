...
//                            } 
                        ...
                        Elements linkHeaders2 = doc2.select("div.box_photo a");
                        for (int z = 0; z <=19; z++) 
                        {
                        	//boolean valueAlreadySet = false;
                        	if (!linkHeaders2.isEmpty() && z < linkHeaders2.size()) 
                            {
                        		String srcValue = linkHeaders2.get(z).select("img").attr("src");
                                String piccFieldName = "picc" + (z + 1);
                                    
                                try 
                                {
                                	vo.setField(piccFieldName, srcValue);
                                    System.out.println(z + 1 + ": " + srcValue);
                                    //valueAlreadySet = true;
                                 } catch (Exception ex) 
                                 {
                                    ex.printStackTrace();
